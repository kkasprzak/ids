...
type WorkbookCellValue = str | int | float | Decimal | datetime | None
type WorkbookRowOverrides = dict[str, WorkbookCellValue]
...
def make_xlsx(  # noqa: PLR0913
    ...
    *,
    ...
    equity: Decimal,
    export_dt: datetime,
    positions: list[WorkbookRowOverrides],
    closed_positions: list[WorkbookRowOverrides],
) -> None:
    """Write a minimal XTB-like workbook for adapter tests."""
    workbook = Workbook()
    default: Worksheet | None = workbook.active
    assert default is not None
    workbook.remove(default)

    closed_sheet = _create_sheet(workbook, "CLOSED POSITION HISTORY")
    open_sheet = _create_sheet(workbook, f"OPEN POSITION {export_dt.strftime('%d%m%Y')}")
    _create_sheet(workbook, "PENDING ORDERS HISTORY")
    _create_sheet(workbook, "CASH OPERATION HISTORY")
