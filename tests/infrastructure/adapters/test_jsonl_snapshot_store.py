...
def test_round_trip(
    ...
    snapshot = make_snapshot(positions=(make_position(id=7), make_position(id=8)))
    store = _store(tmp_path)
    store.save(snapshot)
...
def test_unsupported_schema_version_raises(tmp_path: Path) -> None:
    ...
    path.write_text(
        '{"schema_version":2,"as_of_date":"2026-05-02","source_id":"x","account":{"balance_pln":"1","equity_pln":"1","export_datetime":"2026-05-02T10:00:00+02:00"},"positions":[]}\n',
        encoding="utf-8",
    )
